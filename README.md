...
The simulator is for pre-programming debugging. Technically, the simulator an entirely separate program from Quartus. You can use any simulator available online, but we chose to use ModelSim since it is optional add-on when you download Quartus. More later.
...
The Signal Tap Logic Analyzer is for post-programming debugging. More later.
